...
// 1. Eliminar solo asignaciones existentes
try {
    $delete_aula = $pdo->exec("DELETE FROM schedule_assignments WHERE estado = 'activo'");
...
function distribuirMateriasEnSemana($pdo, $group, &$subjects, $horario_turno, $dias_turno, &$mensajes_error, $horarios_disponibles)
{
    ...
    // Para controlar cuántas horas seguidas lleva cada materia en cada día:
        ...
        foreach ($subjects as &$subject) {
            ...
            while ($subject['remaining_hours'] > 0 && $hora < $fin_turno) {
                ...
                if (asignarBloqueHorario($pdo, $subject, $group, $dia, $hora, $hora+3600, $mensajes_error)) {
                    ...
                    // no se pudo, intentar con otro profesor si es posible
                    if ($subject['teacher_id'] !== null) {
                        // Buscar otro profesor disponible para esta materia
                        $alternateTeacherStmt = $pdo->prepare("
                            ...
                            LIMIT 1
                        ");
                        $alternateTeacherStmt->execute([
                            ':subject_id' => $subject['subject_id'],
                            ':group_id' => $group['group_id'],
                            ':current_teacher_id' => $subject['teacher_id']
                        ]);
                        $alternateTeacher = $alternateTeacherStmt->fetch(PDO::FETCH_ASSOC);
                        if ($alternateTeacher) {
                            $subject['teacher_id'] = $alternateTeacher['teacher_id'];
                            // Intentar asignar nuevamente con el nuevo profesor
                            if (asignarBloqueHorario($pdo, $subject, $group, $dia, $hora, $hora+3600, $mensajes_error)) {
                                $subject['remaining_hours']--;
                                if (!$esSinRestriccion) {
                                    $consecutivasActuales++;
                    ...
                    // Si no se pudo asignar, avanzar la hora
                    $hora += 3600;
                }
            }
        }
    ...
    // ------------------------------------------------
    // SEGUNDA PASADA: Cualquier espacio disponible
    // ------------------------------------------------
    foreach ($subjects as &$subject) {
...
foreach ($groups as $group) {
    ...
    try {
        ...
        // y dentro de cada grupo, por la que tenga mas 'remaining_hours'.
        usort($subjects, function ($a, $b) {
            $aHasTeacher = !empty($a['teacher_id']);
            $bHasTeacher = !empty($b['teacher_id']);
        ...
        // Llamar a la función que hace la primera y segunda pasada
