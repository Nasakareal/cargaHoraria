...
if (isset($_FILES['file'])) {
    ...
    if (($handle = fopen($file, 'r')) !== FALSE) {
        ...
        while (($data = fgetcsv($handle, 1000, ',')) !== FALSE) {
            ...
            $nombre = mb_strtoupper(trim($data[1])) . ' ' . mb_strtoupper(trim($data[2])) . ' ' . mb_strtoupper(trim($data[3])); // Nombre completo en mayúsculas
            $puesto = mb_strtoupper(trim($data[4])); // Puesto en mayúsculas
            $area = mb_strtoupper(trim($data[5])); // Área en mayúsculas
            $clasificacion = mb_strtoupper(trim($data[7])); // Clasificación en mayúsculas
