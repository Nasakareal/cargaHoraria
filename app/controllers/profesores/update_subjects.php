...
try {
    ...
    $sentencia_programa_cuatrimestre = $pdo->prepare("REPLACE INTO teacher_program_term 
    ...
    /* Actualizar las horas totales del profesor en la tabla teachers */
    $sentencia_actualizar_horas = $pdo->prepare("UPDATE teachers SET hours = :hours, fyh_actualizacion = :fyh_actualizacion WHERE teacher_id = :teacher_id");
    $sentencia_actualizar_horas->bindParam(':hours', $total_hours);
    $sentencia_actualizar_horas->bindParam(':fyh_actualizacion', $fechaHora);
    $sentencia_actualizar_horas->bindParam(':teacher_id', $teacher_id);

    if (!$sentencia_actualizar_horas->execute()) {
        throw new Exception("Error al actualizar las horas del profesor en la tabla teachers: " . implode(", ", $sentencia_actualizar_horas->errorInfo()));
    }
    ...
    foreach ($grupo_ids as $grupo_id) {
        if (!in_array($grupo_id, $grupos_actuales)) {
            $sentencia_insertar_grupo = $pdo->prepare("INSERT INTO teacher_groups (teacher_id, group_id, fyh_creacion)
    ...
    foreach ($materia_ids as $materia_id) {
        ...
        foreach ($grupo_ids as $grupo_id) {
            ...
            $sentencia_actualizar_schedule = $pdo->prepare("UPDATE schedules s
            ...
            $sentencia_eliminar_gst = $pdo->prepare("DELETE gst FROM group_schedule_teacher gst
            ...
            $sentencia_obtener_schedules = $pdo->prepare("SELECT s.schedule_id FROM schedules s
            ...
            foreach ($schedule_ids as $schedule_id) {
                $sentencia_insertar_gst = $pdo->prepare("INSERT INTO group_schedule_teacher (schedule_id, teacher_id, subject_id, fyh_creacion)
